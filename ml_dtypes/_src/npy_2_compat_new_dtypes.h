...
static inline int PyArrayInitDTypeMeta_FromSpec(PyArray_DTypeMeta* DType,
  ...
  int bad_offset =
      (PyArray_RUNTIME_VERSION >= NPY_2_4_API_VERSION) ? (1 << 10) : (1 << 11);
  int good_offset =
      (PyArray_RUNTIME_VERSION >= NPY_2_4_API_VERSION) ? (1 << 11) : (1 << 10);
  ...
  PyErr_SetString(PyExc_RuntimeError,
                  "NPY_DT_legacy_descriptor_proto backport not supported in "
                  "Python limited API");
  return -1;
#else
